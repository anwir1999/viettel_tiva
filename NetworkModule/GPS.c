...
parse_term(gps_t* gh) {
    ...
    if (gh->p.stat == STAT_UNKNOWN) {
        ...
        switch (gh->p.term_num) {
            ...
            //     if (gh->p.data.rmc.course == 0)
            //     {
            //          DebugUart("\r\neror\r\n");
            //     }                           /* Process true ground coarse */
...
gps_process(gps_t* gh, const void* data, size_t len) {
    ...
    for (; len > 0; ++d, --len) {               /* Process all bytes */
        if (*d == '$') {                        /* Check for beginning of NMEA line */
            ...
            TERM_ADD(gh, *d);
            #ifdef DEBUG_PARSE_GPS
            DebugUart("Init GPS success1\r\n");                   /* Add character to term */
            #endif
        } else if (*d == ',') {                 /* Term separator character */
            parse_term(gh);
                      /* Parse term we have currently in memory */
            CRC_ADD(gh, *d);                    /* Add character to CRC computation */
            TERM_NEXT(gh);
            #ifdef DEBUG_PARSE_GPS
             DebugUart("Init GPS success2\r\n");                     /* Start with next term */
            #endif
        }
        else if (*d == '*')
        {
            #ifdef DEBUG_PARSE_GPS
            DebugUart("Init GPS success31\r\n");                /* Start indicates end of data for CRC computation */
            #endif
            parse_term(gh);                     /* Parse term we have currently in memory */
            gh->p.star = 1;                     /* STAR detected */
            TERM_NEXT(gh);
            #ifdef DEBUG_PARSE_GPS
            DebugUart("Init GPS success3\r\n");                     /* Start with next term */
            #endif
        }
        else if (*d == '\r') {
            if (check_crc(gh)) {                /* Check for CRC result */
                /* CRC is OK, in theory we can copy data from statements to user data */
                copy_from_tmp_memory(gh);
                #ifdef DEBUG_PARSE_GPS
                DebugUart("Init GPS success4\r\n");      /* Copy memory from temporary to user memory */
                #endif
#if GPS_CFG_STATUS
                if (evt_fn != NULL) {
                    evt_fn(gh->p.stat);
